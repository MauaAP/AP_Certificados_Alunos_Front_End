    ...
    const handleSubmit = async (pessoaId) => {
        if (!pessoaId) {
            if (!name || !email) {
                ...
                try {
                    setLoading(true);
                    const response = await axios.post("https://ap-certificados-alunos-back-end.onrender.com/api/create-external-presence", {
                        name: name,
                        email: email,
                        eventId: eventId
                    });
        ...
        try {
            setLoading(true);
            const response = await axios.post("https://ap-certificados-alunos-back-end.onrender.com/api/create-presence", {
                userid: pessoaId,
                eventid: eventId
            });
